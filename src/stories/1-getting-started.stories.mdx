...
The `mg-components` components are now globally available in you project, no need to import them individually in your Vue.js views or components.
...
<mg-input-text
  ...
  :label="$t('firstname')"
  @valueChange="user.firstname = $event.target.value"
></mg-input-text>
```
