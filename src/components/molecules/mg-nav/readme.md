...
| `identifier`         | `identifier`  | Identifier is used for the element ID (id is a reserved prop in Stencil.js) If not set, it will be created. | `string`                | `createID('mg-nav')` |
...
graph TD;
  mg-nav --> mg-icon
  mg-nav --> mg-badge
  style mg-nav fill:#f9f,stroke:#333,stroke-width:4px
```
