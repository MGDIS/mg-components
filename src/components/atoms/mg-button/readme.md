...
| `loading`        | `loading`          | Define if button is loading, default to false. Trigger when button is clicked or key-up ['enter', 'space], then value change to true. It's required to reset to false when action/promise in parent is done to stop the loading state | `boolean` | `false`                 |
